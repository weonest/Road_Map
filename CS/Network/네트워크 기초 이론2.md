...
## 21. Proxy의 활용 첫 번째, ‘우회’

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/a1cf92ad-c7a3-4977-973d-318e71b93383/Untitled.png)

- Tor Project = 인터넷에게 자유를..
- 우회를 할 때 중요한 점 : Proxy를 통해 우회를 하는 순간 `Proxy 대상자`는 `Proxy 설정자`의 모든 통신을 감시할 수 있다
- VPN은 Network 수준에서, Proxy는 HTTP 수준에서

## 22. Proxy의 활용 두 번째, ‘분석’

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/44ad1b1c-a676-4d7d-a862-a57a41b34fc3/Untitled.png)

- 웹으로 통신을 한다면 기본적으로 HTTP 통신
  - HTTP는 암호화 되지 않은 평문이므로 나의 정보가 그대로 노출
  - `SSL (Secure Sockets Layer)` 를 적용하여 Socket - Stream 데이터에 `PKI (Public Key Infrastructure)` 기술을 적용시켜 **암호화를 시킨 것**이 **HTTPS**이다
  - 서버로부터 전달받는 데이터는 Socket을 통해 하위 계층으로 내려오는데, 제일 상위 계층인 User mode Application 레이어에서 HTTP에 SSL이 적용된다
- Wireshark 프로그램을 통해 나와 서버 간의 통신상태를 분석하거나 모니터링 해야할 때가 있다
  - 위에서 전달받은 HTTPS가 하위 계층으로 전달되어 Wireshark가 Sensor가 수집한 Packet의 데이터 부분 (Packet은 Header와 Data (Payload) 부분으로 나누어져 있다.) 을 분석하려고 할 경우, 이미 암호화된 데이터를 해석하기에는 어려움이 있다.
  - 해석할 수 있는 방법이 없는 것은 아니지만, HTTPS는 단위가 Stream이고, Wireshark의 Sensor가 수집한 데이터의 단위는 Packet이다. 서로 다른 단위의 암호화 된 데이터를 분석하는 것은 비효율적
  - 이러한 경우를 해결하기 위해 **Proxy 서버**를 이용한다. 서버로부터의 Steram 데이터가 암호화 되어 하위 계층으로 이동하기 전에, Proxy 서버로 데이터를 전달하는 것
  - 이때 Proxy 서버의 IP주소를 로컬 주소 (자기 자신의 IP주소)로 설정한다. 이 경우는 내가 나의 정보를 보는 것이므로 아무런 문제가 되지 않는다
  - Proxy 서버는 평문의 HTTP Stream 데이터를 받은 뒤 ‘암호화’한다. 그러고 나서 Proxy 서버의 Socket을 통해 암호화 된 데이터가 하위계층으로 이동하는 것
- Fiddler를 통해 Socket 통신을 스스로 분석할 수 있다. 또, PC, 모바일 모드 등의 환경으로 조작할 수도 있어 개발자들에게 유용하다고 함

## 23. Proxy의 활용 세 번째, ‘감시와 보호’

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/19d34e08-7210-4591-b800-6db45158796d/Untitled.png)

- 전제조건
  - 네트워크 대역이 3.3.3.X (보통 Gateway가 1번)
  - 모든 PC가 Proxy 설정을 해놓은 상태
- PC 1번이 HTTPS 통신으로 특정 사이트에 접속할 경우, Proxy 서버를 통해 접속하게 된다. 악성 코드가 존재하는 사이트에 접속한다면 Proxy 서버를 통해 외부에서 악성코드가 유입되는 것을 차단
- Proxy 서버가 보호 기능을 할 경우 Proxy 서버를 Virus Wall 이라고 부를 수 있다
- Proxy 서버의 감시 기능으로는 회사에서 특정 사이트로 접속하지 못하도록 차단할 수도 있다
  - 이를 인터넷 접근 통제 시스템이라고 부를 수도 있다

## 24. Proxy의 활용 네 번째, ‘Reverse Proxy’

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/c9767318-44d0-4310-bcc6-f7661442ba25/Untitled.png)

- 웹 서버 쪽에서 작동하는 Proxy 서버를 ‘Reverse Proxy’라고 한다

  - 클라이언트 쪽에서 작동하는 Proxy 서버는 ‘Forward Proxy’

- 리버스 프록시가 있는 경우에는 클라이언트가 특정 웹 서버로 직접 접속하는 것이 아님

  - 리버스 프록시가 웹 서버로부터 응답을 전달받고, 그것을 다시 클라이언트에게 전달해주는 것
  - 당연히 클라이언트가 접속한 IP 주소도 실제 웹 서버의 IP 주소가 아니라 리버스 프록시 서버의 IP 주소

- 인터넷은 Public 네트워크인데 리버스 프록시와 웹 서버를 

  Private

   네트워크로 분리함

  - 이렇게 해서 외부에서 리버스 프록시와 웹 서버에 접근할 수가 없도록 보호

- 리버스 프록시 서버 또한 포워드 프록시처럼 감시와 보호의 기능을 함

  - 접속한 클라이언트가 무엇을 요청하는지 감시해서 서버에 대한 공격인지 식별하여 차단
  - 이 기능을 하는 프록시 서브를 WAF (Web Application Firewall) 라고 함
  - WAF가 감시하는 트래픽의 단위는 HTTP 통신 단위인 Socket - Stream (L7)이다
    - 이런 크기의 파일을 아주 작은 Packet 단위로 분리하여 감시하는 것보다 WAF를 통해 Stream 단위로 감시하는 것이 좋다