...
좀 더 쉽게 푸는 방법으로는 `bitCount()` 메소드를 이용할 수 있는 것 같다.
...
class Solution {
    public int[] solution(int n, String[] words) {
        ...
        for (int i = 1; i < words.length; i++) {
            ...
            String cur = words[i];
            if (!list.contains(cur) && last.charAt(last.length() - 1) == cur.charAt(0)) {
                list.add(cur);
            } else {
                answer[0] = i % n + 1;
                answer[1] = i / n + 1;
...
현재값과 다음값이 같으면 pop을 해주고 아니면 push를 해줌으로써, 최종적으로 stack 내부가 비어있는지 아닌지만 판별하면 되는 문제였다.
...
최소 보트의 개수를 구하는 것이 초점이었는데, 최소 몸무게인 인원부터 태우려고 해서 조금 헤맸다.
